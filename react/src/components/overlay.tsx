    ...
    <>
      ...
      {open && (
        <div className="fixed inset-0 z-50 grid place-items-center p-4" role="dialog" aria-modal>
          ...
          <div className="glass relative w-full max-w-md rounded-[var(--radius-lg)] border border-border p-6 shadow-[var(--shadow-glass)]">
...
export function DropdownMenu({ label, items }: { label: React.ReactNode; items: { label: string; onClick?: () => void }[] }) {
  ...
  return (
    <div ref={ref} className="relative inline-block">
      ...
      {open && (
        <div className="glass absolute end-0 z-20 mt-2 w-44 rounded-[var(--radius)] border border-border p-1.5 shadow-[var(--shadow-glass)]">
          {items.map((it) => (
            <button
              key={it.label}
              onClick={() => {
